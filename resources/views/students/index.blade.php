                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" style="gap:0.3rem;">
                                    <a href="{{ route('students.show', $student->id) }}" class="btn btn-secondary btn-icon-sm" title="View Profile">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                    </a>
                                    <a href="{{ route('academics.report-card', $student->id) }}" class="btn btn-secondary btn-icon-sm" title="Academic Report Card">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                                    </a>
                                    @if(in_array(Auth::user()->role->slug, ['admin','head-teacher']))
                                        <a href="{{ route('students.edit', $student->id) }}" class="btn btn-secondary btn-icon-sm" style="color:var(--primary-color); border-color:rgba(99,102,241,0.3);" title="Edit Student">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                        </a>
                                    @endif
                                    @if(in_array(Auth::user()->role->slug, ['bursar','accountant','admin']))
                                        <a href="{{ route('finance.invoices', $student->id) }}" class="btn btn-secondary btn-icon-sm" style="color:#a855f7; border-color:rgba(168,85,247,0.3);" title="Student Invoices & Fees">
                                    ...
                                    @if(Auth::user()->role->slug === 'admin')
                                        <form action="{{ route('students.destroy', $student->id) }}" method="POST" onsubmit="return confirm('Delete {{ $student->full_name }}? This cannot be undone.')" style="display:inline;">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn btn-secondary btn-icon-sm" style="color:var(--danger-color); border-color:rgba(239,68,68,0.25);" title="Delete Student">

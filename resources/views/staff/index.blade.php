                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" style="gap:0.3rem;">
                                    <a href="{{ route('staff.show', $member->id) }}" class="btn btn-secondary btn-icon-sm" title="View Staff Profile">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                    </a>
                                    @if(Auth::user()->role->slug === 'admin')
                                        <a href="{{ route('staff.edit', $member->id) }}" class="btn btn-secondary btn-icon-sm" style="color:var(--primary-color); border-color:rgba(99,102,241,0.3);" title="Edit Staff Record">
                                        ...
                                        <form action="{{ route('staff.destroy', $member->id) }}" method="POST" onsubmit="return confirm('Delete this staff record?')" style="display:inline;">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn btn-secondary btn-icon-sm" style="color:var(--danger-color); border-color:rgba(239,68,68,0.25);" title="Delete Staff">
